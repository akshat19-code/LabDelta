  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/40 dark:bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 md:p-6 animate-fade-in">
      <div className="bg-white dark:bg-[#111726] rounded-3xl shadow-2xl border border-stone-200/90 dark:border-slate-800 w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden animate-scale-in">
        {/* Modal Header */}
        <div className="px-6 py-5 border-b border-stone-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-stone-50/40 dark:bg-slate-900/40">
          <div>
          ...
          <button
            onClick={handleClose}
            className="w-9 h-9 rounded-full bg-stone-100 dark:bg-slate-800 hover:bg-stone-200 dark:hover:bg-slate-700 text-stone-500 dark:text-slate-400 hover:text-stone-800 dark:hover:text-slate-200 flex items-center justify-center text-base font-bold transition-colors cursor-pointer"
          >
            ✕
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-5">
          {errorMessage && (
            <div className="p-4 bg-rose-50 dark:bg-rose-950/40 border border-rose-200/80 dark:border-rose-900/60 rounded-2xl text-sm text-rose-700 dark:text-rose-300 font-medium flex items-start space-x-2.5 animate-fade-in">
          ...
          {mode === 'select' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
              ...
              <div
                ...
                className="spotlight-surface border-2 border-[#5B3FE0]/30 hover:border-[#5B3FE0] bg-violet-50/20 dark:bg-violet-950/20 hover:bg-violet-50/50 dark:hover:bg-violet-950/40 rounded-2xl p-6 sm:p-7 text-center space-y-4 transition-all cursor-pointer card-interactive group shadow-2xs relative"
              >
                <div className="w-16 h-16 rounded-2xl bg-[#5B3FE0]/10 dark:bg-[#5B3FE0]/20 text-[#5B3FE0] text-3xl flex items-center justify-center mx-auto transition-transform duration-200 group-hover:scale-110 group-hover:rotate-2">
                  📄
                </div>
                <div className="space-y-1.5 relative z-10">
                  <div className="flex items-center justify-center space-x-2">
                    <h4 className="text-lg sm:text-xl font-bold text-stone-900 dark:text-slate-100">Upload PDF</h4>
                    <span className="text-xs font-bold uppercase tracking-wider bg-[#5B3FE0] text-white px-2.5 py-0.5 rounded-full">
                      AI Powered
                    </span>
                  </div>
                ...
                <button
                  type="button"
                  className="w-full py-3 px-5 btn-primary text-sm sm:text-base font-semibold shadow-xs transition-all cursor-pointer flex items-center justify-center space-x-1.5 relative z-10"
                >
              ...
              <div
                ...
                onMouseMove={handleSpotlightMouseMove}
                className="spotlight-surface border-2 border-stone-200 dark:border-slate-800 hover:border-[#5B3FE0]/40 bg-stone-50/40 dark:bg-slate-900/40 hover:bg-stone-50/80 dark:hover:bg-slate-800/60 rounded-2xl p-6 sm:p-7 text-center space-y-4 transition-all cursor-pointer card-interactive group shadow-2xs relative"
              >
                <div className="w-16 h-16 rounded-2xl bg-stone-100 dark:bg-slate-800 text-stone-700 dark:text-slate-300 text-3xl flex items-center justify-center mx-auto transition-transform duration-200 group-hover:scale-110 group-hover:-rotate-2">
                  ✍️
                </div>
                <div className="space-y-1.5 relative z-10">
                  <h4 className="text-lg sm:text-xl font-bold text-stone-900 dark:text-slate-100">Add Values Manually</h4>
                  <p className="text-xs sm:text-sm text-stone-500 dark:text-slate-400 leading-relaxed">
                ...
                <button
                  type="button"
                  className="w-full py-3 px-5 btn-secondary text-sm sm:text-base font-semibold shadow-xs transition-all cursor-pointer flex items-center justify-center space-x-1.5 relative z-10"
                >
          ...
          {mode === 'upload' && (
            <div className="space-y-5 py-2">
                ...
                <div className="space-y-2.5">
                  ...
                  {selectedFile ? (
                    <div className="space-y-1">
                      <p className="text-base font-bold text-stone-900 dark:text-slate-100 font-mono break-all">{selectedFile.name}</p>
              ...
              {extracting && (
                <div className="bg-stone-50 dark:bg-slate-900/90 p-5 sm:p-6 rounded-2xl border border-stone-200 dark:border-slate-800 space-y-4 animate-fade-in">
                  <div className="flex items-center justify-between text-sm font-semibold text-stone-700 dark:text-slate-300">
                  ...
                  <div className="relative pt-2 pb-1">
                    <div className="absolute top-5 left-4 right-4 h-1 bg-stone-200 dark:bg-slate-800 rounded-full z-0"></div>
                    <div
                      className="absolute top-5 left-4 h-1 bg-[#5B3FE0] rounded-full z-0 stepper-bar-fill"
                    ...
                    <div className="relative z-10 flex justify-between">
                      {stepLabels.map((label, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex flex-col items-center space-y-1.5 w-16">
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                                isDone
                                  ? 'bg-[#5B3FE0] text-white shadow-xs'
                                  : isCurrent
                                  ? 'bg-white dark:bg-[#1E293B] border-2 border-[#5B3FE0] text-[#5B3FE0] dark:text-[#8266FA] shadow-md ring-4 ring-[#5B3FE0]/20 animate-pulse-ring'
              ...
              <div>
                <button
                  ...
                  className="w-full py-3.5 px-5 btn-primary text-sm sm:text-base font-semibold flex items-center justify-center space-x-2"
                >
                  {extracting ? (
                    <>
          ...
          {(mode === 'verify' || mode === 'manual') && (
            <form id="report-entry-form" onSubmit={handleSaveReport} className="space-y-6">
              ...
              {mode === 'verify' && (
                <div className="bg-amber-50/90 border border-amber-200/80 rounded-2xl p-4 sm:p-5 text-sm text-amber-900 dark:bg-amber-950/40 dark:border-amber-800/60 dark:text-amber-200 space-y-1">
              ...
              {extractionNotice && (
                <div className="p-4 bg-violet-50 border border-violet-200 dark:bg-[#5B3FE0]/15 dark:border-[#5B3FE0]/30 dark:text-violet-300 rounded-2xl text-sm text-violet-800 font-medium">
                  {extractionNotice}
                </div>
              )}
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-stone-600 dark:text-slate-400">
                    Report Details
                  </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-stone-50/70 dark:bg-[#131B2E] p-4 sm:p-5 rounded-2xl border border-stone-200/80 dark:border-slate-800">
                  <div className="space-y-1.5 text-left">
                    <label className="block text-xs sm:text-sm font-bold uppercase tracking-wider text-stone-600 dark:text-slate-300">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white dark:bg-[#0B0F19] border border-stone-200 dark:border-slate-700 rounded-xl text-base text-stone-900 dark:text-slate-100 placeholder-stone-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#5B3FE0]/30 focus:border-[#5B3FE0]"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    <input
                      ...
                      required
                      className={`w-full px-4 py-3 bg-white dark:bg-[#0B0F19] border rounded-xl text-base text-stone-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#5B3FE0]/30 focus:border-[#5B3FE0] ${
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                  <h4 className="text-sm sm:text-base font-bold uppercase tracking-wider text-stone-700 dark:text-slate-200">
                    {mode === 'verify'
                      ? `Extracted Measurements (${rows.length})`
                      : `Report Measurements (${rows.length})`}
                  </h4>
                  <span className="text-xs sm:text-sm text-stone-400 dark:text-slate-500">Values can be numeric or categorical</span>
                </div>
                ...
                <div className="space-y-3">
                  {rows.map((row, idx) => (
                    <div
                      key={row.id}
                      className={`p-4 sm:p-5 bg-stone-50/50 dark:bg-[#131B2E]/60 border border-stone-200/90 dark:border-slate-800 rounded-2xl space-y-3 ${
                    ...
                    >
                      <div className="flex items-center justify-between text-xs sm:text-sm text-stone-500 dark:text-slate-400 font-medium">
                        <span className="font-semibold">Measurement #{idx + 1}</span>
                        <button
                          type="button"
                          onClick={() => handleRemoveRow(row.id)}
                          className="text-stone-400 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400 text-xs sm:text-sm font-medium cursor-pointer p-1 transition-colors"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3">
                        ...
                        <div className="sm:col-span-2 md:col-span-4">
                          <label className="block text-xs uppercase font-bold text-stone-500 dark:text-slate-400 mb-1">
                            Test Name
                          </label>
                          <input
                            type="text"
                            ...
                            className="w-full px-3.5 py-2.5 bg-white dark:bg-[#0B0F19] border border-stone-200 dark:border-slate-700 rounded-xl text-base text-stone-900 dark:text-slate-100 placeholder-stone-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#5B3FE0]/30 focus:border-[#5B3FE0] transition-all"
                          />
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3 pt-1">
                        <label className="text-xs sm:text-sm text-stone-600 dark:text-slate-400 font-medium shrink-0">
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2 bg-white dark:bg-[#0B0F19] border border-stone-200 dark:border-slate-700 rounded-xl text-sm text-stone-900 dark:text-slate-100 placeholder-stone-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#5B3FE0]/30 focus:border-[#5B3FE0] transition-all"
                        />
                      </div>
                    </div>
                  ))}
        ...
        {/* Modal Footer */}
        <div className="px-6 py-4 border-t border-stone-100 dark:border-slate-800 flex items-center justify-between bg-stone-50/50 dark:bg-[#0E1526] shrink-0">
          {mode === 'select' && (
            <button
              type="button"
              onClick={handleClose}
              className="btn-secondary ml-auto px-5 py-2.5 text-sm font-semibold text-stone-700 dark:text-slate-200 bg-white dark:bg-[#131B2E] border border-stone-200 dark:border-slate-700 rounded-xl cursor-pointer"
            >
              Close
            </button>
          )}
          ...
          {mode === 'upload' && (
            <>
              <button
                ...
                className="btn-secondary px-5 py-2.5 text-sm font-semibold text-stone-700 dark:text-slate-200 bg-white dark:bg-[#131B2E] border border-stone-200 dark:border-slate-700 rounded-xl cursor-pointer"
              >
                ← Back
              </button>
              <button
                type="button"
                onClick={handleClose}
                className="btn-ghost px-4 py-2.5 text-sm font-semibold text-stone-500 hover:text-stone-800 dark:text-slate-400 dark:hover:text-slate-200 cursor-pointer rounded-xl"
              >
                Cancel
              </button>
            </>
            ...
            <>
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <button
                  ...
                  className="btn-primary px-5 py-2.5 sm:py-3 text-white font-semibold text-sm sm:text-base rounded-xl shadow-xs transition-all flex items-center space-x-2 cursor-pointer disabled:opacity-60"
                >
                  {isSubmitting ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
