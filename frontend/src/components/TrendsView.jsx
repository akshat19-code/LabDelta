  ...
  return (
    <div className="space-y-6 sm:space-y-7 antialiased">
      ...
      <div className="border-b border-stone-200/80 dark:border-slate-800 pb-4 sm:pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        ...
        <button
          onClick={onOpenAddReport}
          className="btn-primary inline-flex items-center justify-center space-x-1.5 px-4 py-2 sm:py-2.5 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-xs cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3.5">
        <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-2xl p-4 sm:p-4.5 lg:p-5 card-interactive space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-stone-400 dark:text-slate-500 font-medium">Tests with ≥2 repeated points</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-2xl p-4 sm:p-4.5 lg:p-5 card-interactive space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-stone-400 dark:text-slate-500 font-medium">Total historical reports</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-2xl p-4 sm:p-4.5 lg:p-5 card-interactive space-y-1">
          ...
          <div className="text-base sm:text-lg font-extrabold text-stone-900 dark:text-slate-100 truncate pt-0.5">
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-2xl p-4 sm:p-4.5 lg:p-5 card-interactive space-y-1">
          ...
          <div className="text-2xl sm:text-3xl lg:text-4xl font-black text-emerald-600 dark:text-emerald-400 font-mono">
            {formatValue(activeUnitGroup?.pointsCount || 0)}
          </div>
          <p className="text-[11px] sm:text-xs text-stone-400 dark:text-slate-500 font-medium">Observations on trend line</p>
        </div>
      </div>

      {/* 3. Searchable Test Selector Strip */}
      <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-3xl p-4 sm:p-5 lg:p-6 shadow-xs space-y-3.5">
          ...
          {summaryData.eligibleTests.length > 3 && (
            <div className="w-full sm:w-60">
              <input
                ...
                className="w-full px-3 py-1.5 bg-stone-50 dark:bg-slate-900/90 border border-stone-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm text-stone-900 dark:text-slate-100 placeholder-stone-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#5B3FE0] focus:border-[#5B3FE0] transition-all"
              />
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-0.5">
          {filteredTests.map((test) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-150 cursor-pointer flex items-center space-x-1.5 btn-press ${
                  isSelected
          ...
          {filteredTests.length === 0 && (
            <p className="text-xs sm:text-sm text-stone-400 dark:text-slate-500 py-1">No matching tests found.</p>
          )}
        </div>
      </div>
      ...
      {currentTest && activeUnitGroup && (
        <div className="bg-white dark:bg-[#131B2E] border border-stone-200/90 dark:border-slate-800 rounded-3xl shadow-xs overflow-hidden space-y-5 p-4.5 sm:p-5.5 lg:p-6.5">
          {/* Chart Header */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 border-b border-stone-100 dark:border-slate-800 pb-3.5">
            <div>
              <div className="flex items-center space-x-2.5">
                ...
                {activeUnitGroup.unit && (
                  <span className="px-2.5 py-0.5 text-xs font-mono font-semibold bg-stone-100 dark:bg-slate-800 text-stone-600 dark:text-slate-300 rounded-lg">
                    {activeUnitGroup.unit}
                  </span>
                )}
              </div>
            ...
            {currentTest.unitGroups.length > 1 && (
              <div className="flex flex-wrap items-center gap-1.5 bg-stone-50 dark:bg-slate-900/80 p-1.5 rounded-2xl border border-stone-200 dark:border-slate-800">
                <span className="text-xs font-semibold text-stone-500 dark:text-slate-400 pl-1.5">Unit:</span>
                {currentTest.unitGroups.map((g) => (
                  <button
                    key={g.unitLabel}
                    onClick={() => setSelectedUnit(g.unitLabel)}
                    className={`min-h-[44px] sm:min-h-0 px-3 py-1.5 sm:py-1 text-xs font-semibold rounded-xl transition-all cursor-pointer btn-press flex items-center ${
          ...
          {excludedInfo && (
            <div className="p-3.5 bg-amber-50/90 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-900/60 rounded-2xl text-xs sm:text-sm text-amber-900 dark:text-amber-300 flex items-start space-x-2 animate-fade-in">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-stone-50/70 dark:bg-[#0F172A]/70 p-4 rounded-2xl border border-stone-200/60 dark:border-slate-800 text-xs">
            {/* First Value */}
            <div>
              <span className="text-[11px] sm:text-xs font-bold text-stone-500 dark:text-slate-400 uppercase tracking-wider block">
                First Recorded
              </span>
              <div className="font-bold text-stone-900 dark:text-slate-100 mt-0.5 font-mono text-sm sm:text-base">
                {formatValue(activeUnitGroup.first.value)} {activeUnitGroup.unit}
              </div>
              <span className="text-[11px] text-stone-400 dark:text-slate-500 font-mono mt-0.5 block">
            ...
            <div>
              ...
              <div className="font-black text-stone-900 dark:text-slate-100 mt-0.5 font-mono text-sm sm:text-base flex items-center space-x-1.5">
              ...
              <span className="text-[11px] text-stone-400 dark:text-slate-500 font-mono mt-0.5 block">Latest − First</span>
            </div>
            ...
            <div>
              ...
              <span className="text-[11px] text-stone-400 dark:text-slate-500 font-mono mt-0.5 block">Relative trajectory</span>
            </div>
          </div>

          {/* Recharts Responsive Line Chart with Animation */}
          <div className="w-full pt-3 pb-1 space-y-3">
            <div className="w-full h-72 sm:h-80 lg:h-88">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 pt-3 text-xs text-stone-500 dark:text-slate-400 border-t border-stone-100 dark:border-slate-800">
          ...
          <div className="border-t border-stone-100 dark:border-slate-800 pt-4 sm:pt-5 space-y-3">
            <h4 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-stone-700 dark:text-slate-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3">
              {activeUnitGroup.points.map((pt, idx) => {
                ...
                return (
                  <div
                    key={pt.id || idx}
                    className="bg-stone-50/60 dark:bg-[#0F172A]/60 border border-stone-200/80 dark:border-slate-800 rounded-2xl p-3.5 sm:p-4 text-xs space-y-2 card-interactive"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-1.5">
                        ...
                        {pt.isDuplicateDateAndLab && (
                          <span className="text-[10px] font-semibold px-1 py-0.5 rounded bg-stone-200/80 dark:bg-slate-800 text-stone-600 dark:text-slate-400 font-mono">
                            #{pt.dateLabIndex}
                          </span>
                        )}
                      </div>
                      <span className="font-black text-stone-900 dark:text-slate-100 font-mono text-sm sm:text-base">
